...
def estados():
    ...
    for digitador in digitadores:
            ...
            for archivo in glob.glob(ruta+"\\**", recursive=True):
                    ...
                    in archivo or ".JPG" in archivo or ".JPEG"  in archivo or ".GIF" in archivo):
...
def lineas():
    ...
    fig,ax = plt.subplots()
    #Generamos las barras para el conjunto de hombres
    rects1 = ax.bar(x + width/3, conteo, width, label='Líneas')
    #Añadimos las etiquetas de identificacion de valores en el grafico
    ax.set_ylabel('Cantidad')
